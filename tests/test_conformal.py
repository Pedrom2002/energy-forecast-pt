...
class TestComputeCIHalfWidth:
    ...
    def test_conformal_differs_from_gaussian(self) -> None:
        ...
        assert abs(hw_conf - hw_gauss) > 0.1, (
            f"Expected conformal ({hw_conf:.2f}) ≠ gaussian ({hw_gauss:.2f})"
        )
    ...
    def test_custom_scale_dict_overrides_default(self) -> None:
        ...
        custom = {"Lisboa": 2.0}
        hw_custom, _ = _compute_ci_half_width(
            self.RMSE, "Lisboa", self.HOUR_PEAK, self.Q90, scale_dict=custom
        )
        hw_default, _ = _compute_ci_half_width(
            self.RMSE, "Lisboa", self.HOUR_PEAK, self.Q90, scale_dict=None
        )
    ...
    @pytest.mark.parametrize("hour,expected_scale", [
        (0, 0.85),   # night
        (5, 0.85),   # still night
    ...
    def test_hour_scale_boundaries(self, hour: int, expected_scale: float) -> None:
        ...
        assert abs(hw - expected) < 1e-9, (
            f"hour={hour}: expected hw={expected:.4f}, got {hw:.4f}"
        )
...
class TestHourScaleFactor:
    ...
    @pytest.mark.parametrize("hour,expected", [
        (0, 0.85), (1, 0.85), (5, 0.85),   # night
        (6, 1.0),  (7, 1.0),                # transition morning
        (8, 1.15), (12, 1.15), (19, 1.15),  # peak
        (20, 1.0), (21, 1.0),               # transition evening
        (22, 0.85), (23, 0.85),             # night again
    ])
    def test_all_hour_bands(self, hour: int, expected: float) -> None:
        assert _hour_scale_factor(hour) == expected, (
            f"hour={hour}: expected {expected}, got {_hour_scale_factor(hour)}"
        )
