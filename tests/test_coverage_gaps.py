...
class TestLogger:
    ...
    def test_set_request_id_and_get_request_id(self):
        from src.utils.logger import set_request_id, get_request_id
    ...
    def test_reload_models_no_auth_succeeds_when_no_key_set(self):
        ...
        if response.status_code == 200:
            ...
            raise AssertionError(
                f"Expected 200 or 503 from reload, got {response.status_code}: {response.text}"
            )
...
class TestPredictionErrorPaths:
    ...
    def test_predict_timeout_returns_504(self):
        ...
        with _with_fake_model_store():
            with patch("src.api.main._make_single_prediction", side_effect=_slow_predict):
    ...
    def test_batch_timeout_returns_504(self):
        ...
        assert response.status_code == 504, f"Expected 504 timeout for batch, got {response.status_code}: {response.text}"
        assert response.json()["detail"]["code"] == "PREDICTION_TIMEOUT"
    ...
    def test_batch_exception_returns_500(self):
        ...
        with _with_fake_model_store():
            with patch(
                ...
                response = client.post("/predict/batch", json=[_VALID_PAYLOAD])
        assert response.status_code == 500, f"Expected 500 for batch exception, got {response.status_code}: {response.text}"
        assert response.json()["detail"]["code"] == "PREDICTION_FAILED"
    ...
    def test_explain_timeout_returns_504(self):
        ...
        with _with_fake_model_store():
            with patch("src.api.main._explain_prediction", side_effect=_slow_explain):
                with patch("src.api.main.PREDICTION_TIMEOUT_SECONDS", 0.05):
                    response = client.post("/predict/explain?top_n=5", json=_VALID_PAYLOAD)
        assert response.status_code == 504, f"Expected 504 timeout for explain, got {response.status_code}: {response.text}"
    ...
    def test_explain_exception_returns_500(self):
        ...
        with _with_fake_model_store():
            with patch(
                ...
                response = client.post("/predict/explain?top_n=5", json=_VALID_PAYLOAD)
        assert response.status_code == 500, f"Expected 500 for explain exception, got {response.status_code}: {response.text}"
    ...
    def test_sequential_no_lags_fallback(self):
        ...
        history = [
            {
                "timestamp": (base + pd.Timedelta(hours=h)).isoformat(),
                "region": "Norte",
                "temperature": 12.0, "humidity": 75.0, "wind_speed": 8.0,
                "precipitation": 0.1, "cloud_cover": 60.0, "pressure": 1010.0,
        ...
        forecast = [{
            "timestamp": "2025-01-03T00:00:00",
            "region": "Norte",
            "temperature": 10.0, "humidity": 80.0, "wind_speed": 6.0,
            "precipitation": 0.0, "cloud_cover": 70.0, "pressure": 1008.0,
        }]
        original = getattr(app.state, "models", None)
        ...
        try:
            response = client.post(
                "/predict/sequential", json={"history": history, "forecast": forecast}
            )
        finally:
            app.state.models = original
        if response.status_code == 200:
    ...
    def test_sequential_with_lags_model_only(self):
        ...
        history = [
            {
                "timestamp": (base + pd.Timedelta(hours=h)).isoformat(),
                "region": "Centro",
                "temperature": 13.0, "humidity": 70.0, "wind_speed": 9.0,
                "precipitation": 0.0, "cloud_cover": 55.0, "pressure": 1012.0,
        ...
        forecast = [{
            "timestamp": "2025-01-03T00:00:00",
            "region": "Centro",
            "temperature": 11.0, "humidity": 75.0, "wind_speed": 7.0,
            "precipitation": 0.0, "cloud_cover": 65.0, "pressure": 1009.0,
        }]
        original = getattr(app.state, "models", None)
        if original is not None:
            reduced = copy(original)
    ...
    def test_batch_with_lags_model_falls_back_to_per_item(self):
        ...
        response = client.post(
            "/predict/batch?use_model=with_lags", json=[_VALID_PAYLOAD]
        )
        if response.status_code == 200:
            data = response.json()
...
class TestEvaluationPlots:
    ...
    def test_plot_prediction_intervals_with_save(self, evaluator, sample_arrays, tmp_path):
        ...
        y_lower = y_pred - 100
        y_upper = y_pred + 100
        fig = evaluator.plot_prediction_intervals(
            y_true, y_pred, y_lower, y_upper, save_name="intervals.png"
        )
        assert fig is not None
        plt.close("all")
    ...
    def test_plot_prediction_intervals_truncates(self, evaluator):
        ...
        y_upper = y_pred + 100
        fig = evaluator.plot_prediction_intervals(
            y_true, y_pred, y_lower, y_upper, max_points=100
        )
        assert fig is not None
        plt.close("all")
    ...
    def test_plot_prediction_intervals_with_timestamps(self, evaluator, sample_arrays):
        ...
        ts = pd.date_range("2025-01-01", periods=len(y_true), freq="h")
        fig = evaluator.plot_prediction_intervals(
            y_true, y_pred, y_lower, y_upper, timestamps=ts
        )
        assert fig is not None
        plt.close("all")
